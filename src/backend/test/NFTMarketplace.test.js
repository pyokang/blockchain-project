...
describe("NFTMarketplae", function () {
  const feePercent = 1;
  let deployer, addr1, addr2, nft, marketplace;
  let URI = "Sample URI";

  // Deploy smartcontracts before each test
  ...
  describe("Making marketplace items", function () {
    beforeEach(async function () {
      await nft.connect(addr1).mint(URI);
      // Why do we need approval function?
      // For transferFrom function to work, caller must have approved marketplace to transfer on their behalf
    ...
    it("Should fail if price set to 0", async function () {
      await expect(
